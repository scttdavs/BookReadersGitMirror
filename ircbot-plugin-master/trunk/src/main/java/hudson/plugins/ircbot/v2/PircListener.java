...
//    }
    ...
    public static ArrayList<String> getRecipients(String msg, String noUser) {
        ...
        ret.addAll(Arrays.asList(args.split(",")));
        
	    // dedupe recipients including sender
	    Set<String> hash = new LinkedHashSet<String>(ret);
	    ret.clear();
	    ret.addAll(hash);
	    ret.remove(noUser);
    ...
    // provide IRC formatting of strings (color)
    private void onFormatMessage(GenericMessageEvent<PircBotX> event) {
        ...
        if(tokens.length == 5) {
            ...
            if(color != null) {
                ...
                LOGGER.info("FORMAT " + nick + " " + tokens[3]+" "+ tokens[4]);
            } else {
